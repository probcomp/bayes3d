...
for iter in range(NUM_ITER):
    ...
    object_poses = jax.vmap(
        lambda key: j.distributions.gaussian_vmf(key, 0.00001, 0.001)
    ...
    DATASET_FILE = os.path.join(
        j.utils.get_assets_dir(), f"datasets/{DATASET_FILENAME}"
    )
    np.savez(
        DATASET_FILE,
        rgbds=all_data,
    ...
    rgb_images = j.hstack_images([j.get_rgb_image(r.rgb) for r in all_data]).save(
        f"dataset_{iter}.png"
    )
