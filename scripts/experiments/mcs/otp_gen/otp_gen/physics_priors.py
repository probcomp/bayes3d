...
def physics_prior_v1(prev_pose, prev_prev_pose, bbox_dims, camera_pose, world2cam):
    ...
    prev_prev_pos_world = camera_pose[:3, :] @ jnp.concatenate(
        [prev_prev_pos, 1], axis=None
    )
    ...
    vel_pos_world = jax.lax.cond(
        jnp.less_equal(object_bottom, 0.01 * bbox_dims[2]),
...
def physics_prior_v2(
    prev_poses, bbox_dims, camera_pose, world2cam, T, t_interval=1.0 / 60.0
):
    """
    Score the physics of the simulation outside of a PPL, this will
    score physics estimates independent of what we see (3DP3 likelihood)
