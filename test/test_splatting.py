...
intrinsics = b.Intrinsics(
    height=100, width=100, fx=50.0, fy=50.0, cx=50.0, cy=50.0, near=0.1, far=6.0
)
...
proj_matrix = torch.tensor(
    getProjectionMatrix(intrinsics.near, intrinsics.far, fovX, fovY), device=device
)
...
p_hom = torch.transpose(proj_matrix, 0, 1) @ torch.tensor(
    [means3D[0, 0], means3D[0, 1], means3D[0, 2], 1.0], device=device
)
print(p_hom)
