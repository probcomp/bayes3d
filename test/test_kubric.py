...
for i in tqdm(gt_ids):
    mesh_path = os.path.join(
        model_dir, b.utils.ycb_loader.MODEL_NAMES[i], "textured.obj"
    )
...
intrinsics = b.Intrinsics(
    rgbd.intrinsics.height,
    rgbd.intrinsics.width,
    200.0,
    200.0,
    rgbd.intrinsics.width / 2,
    rgbd.intrinsics.height / 2,
    rgbd.intrinsics.near,
    rgbd.intrinsics.far,
)
print(intrinsics)
...
for i in range(len(gt_ids)):
    poses.append(gt_poses[i] @ b.t3d.inverse_pose(offset_poses[i]))
poses = jnp.array(poses)

rgbds = render_many(
    mesh_paths, poses[None, ...], intrinsics, scaling_factor=1.0, lighting=5.0
)
...
b.multi_panel(
    [kubri_rgb, kubric_depth, rerendered_depth, overlay],
    labels=["kubric_rgb", "kubric_depth", "rerendered_depth", "overlay"],
