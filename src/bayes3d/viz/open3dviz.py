...
def trimesh_to_o3d_triangle_mesh(trimesh_mesh):
    ...
    mesh.triangle_normals = o3d.utility.Vector3dVector(
        np.array(trimesh_mesh.face_normals)
    )
    return mesh
...
class Open3DVisualizer(object):
    def __init__(self, intrinsics):
        self.render = o3d.visualization.rendering.OffscreenRenderer(
        ...
        self.render.scene.set_lighting(
            self.render.scene.LightingProfile.NO_SHADOWS, (0, 0, 0)
        )
    ...
    def make_bounding_box(self, dims, pose, color=None, update=True):
        ...
        )  # or MaterialRecord(), for later versions of Open3D
    ...
    def capture_image(self, intrinsics, camera_pose):
        ...
        center = np.array(camera_pose[:3, 3]) + np.array(
            camera_pose[:3, 2]
        )  # look_at target
