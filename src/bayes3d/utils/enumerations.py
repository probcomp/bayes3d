...
from bayes3d.transforms_3d import transform_from_axis_angle, transform_from_pos
...
def angle_axis_helper_edgecase(newZ):
    ...
    geodesicAngle = jax.lax.cond(
        jnp.allclose(newZ, zUnit, atol=1e-3), lambda: 0.0, lambda: jnp.pi
    )
...
def angle_axis_helper(newZ):
    ...
    geodesicAngle = jax.lax.cond(
        jnp.dot(zUnit, newZ) > 0, lambda: theta, lambda: jnp.pi - theta
    )
    return axis, geodesicAngle
...
def geodesicHopf_rotate_within_axis(newZ, planarAngle):
    ...
    return transform_from_axis_angle(axis, geodesicAngle) @ transform_from_axis_angle(
        zUnit, planarAngle
    )
...
def make_rotation_grid_enumeration(
    ...
    unit_sphere_directions = fibonacci_sphere(
        fibonacci_sphere_points, sphere_angle_range
    )
    geodesicHopf_select_axis_vmap = jax.vmap(
        jax.vmap(geodesicHopf_rotate_within_axis, in_axes=(0, None)), in_axes=(None, 0)
    )
    rot_stepsize = (max_rot_angle - min_rot_angle) / num_planar_angle_points
    rotation_proposals = geodesicHopf_select_axis_vmap(
        unit_sphere_directions, jnp.arange(min_rot_angle, max_rot_angle, rot_stepsize)
    ).reshape(-1, 4, 4)
    return rotation_proposals


def make_translation_grid_enumeration(
    min_x, min_y, min_z, max_x, max_y, max_z, num_x=2, num_y=2, num_z=2
):
    """
...
def make_translation_grid_enumeration_3d(
    min_x, min_y, min_z, max_x, max_y, max_z, num_x=2, num_y=2, num_z=2
):
    deltas = jnp.stack(
        jnp.meshgrid(
...
def make_pose_grid_enumeration(
    min_x,
    min_y,
    min_z,
    min_rotation_angle,
    ...
    rotations = make_rotation_grid_enumeration(
        fibonacci_sphere_points,
        num_planar_angle_points,
        min_rotation_angle,
        max_rotation_angle,
    ...
    translations = make_translation_grid_enumeration(
        min_x, min_y, min_z, max_x, max_y, max_z, num_x, num_y, num_z
    )
    all_proposals = jnp.einsum("aij,bjk->abik", rotations, translations).reshape(
        -1, 4, 4
    )
