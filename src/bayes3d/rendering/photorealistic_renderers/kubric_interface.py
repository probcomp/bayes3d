...
def render_many(
        ...
        for object_index in range(poses.shape[1]):
            poses_pos_quat.append(
                (
                    ...
                    np.array(
                        j.t3d.rotation_matrix_to_quaternion(
                            poses[scene_index, object_index, :3, :3]
                        )
                    ),
                )
            )
    ...
    cam_pose_pos_quat = (
        ...
        np.array(j.t3d.rotation_matrix_to_quaternion(camera_pose[:3, :3])),
    )
    ...
    command_strings = "".join(
        [
            f""" --volume {os.path.dirname(p)}:{os.path.dirname(p)} """
            for p in mesh_paths
        ]
    )
