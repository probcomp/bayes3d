...
ildj_registry_rules = core.ildj_registry_rules
NDSlice = slc.NDSlice
...
class CustomInverse:
    ...
    def def_inverse_and_ildj(self, f_ildj):
        ...
        def ildj_rule(incells, outcells, *, in_tree, out_tree, num_consts, **_):
            ...
            if all(outcell.top() for outcell in outcells) and any(
                not incell.top() for incell in incells
            ):
                ...
                none_ = object()
                new_invals = tree_util._replace_nones(
                    none_, new_invals
                )  # pylint: disable=protected-access
                new_ildjs = tree_util._replace_nones(
                    none_, new_ildjs
                )  # pylint: disable=protected-access
            ...
            elif all(incell.top() for incell in incells) and any(
        ...
        ildj_registry_rules[self.prim] = ildj_rule
