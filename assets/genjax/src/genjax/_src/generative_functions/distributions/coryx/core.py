...
class InverseAndILDJ(Cell):
    ...
    def top(self) -> bool:
        ...
        An InverseAndILDJ is at the top if its slice represents the
        entire array.
        """
    ...
    def val(self):
        if not self.top():
            raise AssertionError(
                "Cannot get value from non-top lattice value: ",
    ...
    def ildj(self):
        if not self.top():
            raise AssertionError(
                "Cannot get ildj from non-top lattice value: ",
                f"{self.aval}, {self.slices}",
            )
...
def inverse_and_ildj(f, *trace_args, reduce_ildj=True):
    ...
    def wrapped(*args, **kwargs):
        ...
        jaxpr, (_, in_tree, _) = trace_util.stage(f, dynamic=False)(
            *forward_args, **kwargs
        )
        ...
        with propagate.Interpreter.new(InverseAndILDJ, ildj_registry) as interpreter:
            env, _ = interpreter(
                jaxpr.jaxpr,
                flat_constcells,
                flat_incells,
...
def inverse(f, *trace_args, **inverse_kwargs):
    def wrapped(*args, **kwargs):
        return inverse_and_ildj(f, *trace_args, **inverse_kwargs)(*args, **kwargs)[0]
...
def ildj(f, *trace_args, **inverse_kwargs):
    def wrapped(*args, **kwargs):
        return inverse_and_ildj(f, *trace_args, **inverse_kwargs)(*args, **kwargs)[1]
...
def register_elementwise(prim):
    ...
    def make_rule(f):
        ...
        ildj_registry_rules[prim] = ildj_rule
...
def register_binary(prim):
    ...
    def make_rule(f_left, f_right):
        def ildj_rule(incells, outcells, **params):
            ...
            elif not outcell.top() and left.top() and right.top():
...
ildj_registry_rules = InverseDict()
...
def initial_ildj(incells, outcells, *, jaxpr, num_consts, **_):
    ...
    env, state = propagate.propagate(
        InverseAndILDJ, ildj_registry, jaxpr, const_cells, incells, outcells
    )  # pytype: disable=wrong-arg-types
...
def map_ildj(prim, incells, outcells, **params):
    ...
    if "out_axes" in params:
        ...
        new_params["out_axes_thunk"] = jax_util.HashableFunction(
            lambda: (0,) * aux().num_leaves, closure=("ildj", params["out_axes"])
        )
        del new_params["out_axes"]
    flat_out = prim.bind(f, *flat_vals, **new_params)
    out_tree = aux()
    new_incells, new_outcells, state = tree_util.tree_unflatten(out_tree, flat_out)
    new_incells = [add_slice(v, old_v) for old_v, v in safe_zip(incells, new_incells)]
    new_outcells = [
        add_slice(v, old_v) for old_v, v in safe_zip(outcells, new_outcells)
    ]
...
ildj_registry = propagate.PropagationRules(default_rule, ildj_registry_rules)
