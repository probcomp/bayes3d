...
inliers = (jnp.abs(points_in_plane_frame[:,2]) < 0.01)
...
jax3dp3.viz.save_depth_image(images[0,:,:,2], "rendererd.png", max=1.5)

renderered_points_in_table_frame = t3d.apply_transform(
    t3d.point_cloud_image_to_points(images[0,:,:,:]),
    jnp.linalg.inv(table_pose)
)
...
plt.clf()
plt.scatter(gt_points_in_table_frame_filtered[:,0],gt_points_in_table_frame_filtered[:,1])
plt.scatter(renderered_points_in_table_frame[:,0],renderered_points_in_table_frame[:,1])
plt.savefig("scatter.png")
...
for idx in object_indices:
    ...
    proposals = pose_proposals
    images = jax3dp3.render_parallel(proposals, idx)
    weights = scorer_parallel_jit(gt_image_single_object, images, 0.02, 0.1, 1**3)
