...
gt_image = depth_to_point_cloud_image(cv2.resize(depth * (segmentation == 2), (w,h),interpolation=1), fx,fy,cx,cy)
...
rotation_deltas = jax3dp3.enumerations.make_rotation_grid_enumeration(40, 30)
...
for idx in object_indices:
    images = pgl.render(poses_to_score, h,w,idx)
    weights = scorer_parallel_jit(images, gt_image)
    jax3dp3.viz.save_depth_image(images[weights.argmax(),:,:,2], "best_{}.png".format(model_names[idx]), max=max_depth)
