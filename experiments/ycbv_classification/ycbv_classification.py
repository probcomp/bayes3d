...
for idx in range(num_models):
    ...
    weights = scorer_parallel_jit(images, gt_img, r, outlier_prob)
    best_pose_idx = weights.argmax()
  
    all_scores.append(weights[best_pose_idx])
    all_pred_poses.append(poses_to_score[best_pose_idx])

    best_model_img = jax3dp3.viz.get_depth_image(images[best_pose_idx,:,:,2], max=far)
    overlayed = jax3dp3.viz.overlay_image(gt_depth_img, best_model_img, alpha=0.8)

    multi = jax3dp3.viz.multi_panel(
            [rgb_img, best_model_img, gt_depth_img, overlayed],
...
best_model = model_names[pred_ycb_idx]
end= time.time()
 

gt_model_idx = gt_ycb_idx
...
print("FPS >=", 1/(elapsed/num_models))
