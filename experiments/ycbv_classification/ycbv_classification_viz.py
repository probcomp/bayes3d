...
max_r = 20
min_r = 0
likelihood_r_range = [r for r in reversed(np.linspace(5, max_r,10))] + [r for r in reversed(np.linspace(1,5,10))] + [r for r in reversed(np.linspace(min_r,1,20))] 
outlier_prob_range = [0.05] 
...
for params in all_models_best_results[0].keys():  
    likelihood_r, outlier_prob = params
    ...
    pred_ycb_idx = np.argmax(all_scores)

    # initialize panel img + label
    panels = [gt_depth_img]
    labels = [f"GT Image\n ycb model idx={gt_ycb_idx}"]

    # make duplicate panel for best model
    all_pred_poses = [all_pred_poses[pred_ycb_idx]] + all_pred_poses
    all_scores = [all_scores[pred_ycb_idx]] + all_scores
    model_idx = [pred_ycb_idx] + [i for i in range(num_models)]

    # from IPython import embed; embed()


    # make panel img for each model 
    for i, (pred_pose, score) in enumerate(zip(all_pred_poses, all_scores)):
        pred_image_unmasked = jax3dp3.renderer.render_single_object(all_pred_poses[i], model_idx[i])  # unmasked predicted depth image
    ...
    # Add panel to gif 
    dst = jax3dp3.viz.multi_panel(panels, labels, middle_width=10, top_border=75, fontsize=15)
    all_images.append(dst)

    if pred_ycb_idx == gt_ycb_idx:
        for _ in range(5):
            all_images.append(dst)
...
all_images[0].save(
    fp=f"likelihood_panels_gt_{gt_ycb_idx}_pred_{pred_ycb_idx}.gif",
    format="GIF",
