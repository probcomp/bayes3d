...
shape_scale = 2.0
ground_truth_mean = center+np.random.multivariate_normal(mean=[0,0,0],cov=1e-1*np.identity(3)*shape_scale,size=NUM_MIXTURE)
ground_truth_weight_log = jnp.log(np.ones(NUM_MIXTURE)/NUM_MIXTURE)
ground_truth_sphere_size = 14
ground_truth_prec = jnp.array([np.identity(3)*ground_truth_sphere_size/shape_scale for _ in range(NUM_MIXTURE)])
...
camera_angle_sweep = jnp.linspace(0.0, jnp.pi, 10)
distance = 5.0
...
for angle in camera_angle_sweep:
    ...
    ground_truth_depth,_,ground_truth_alpha = fm_render.render(ground_truth_mean, ground_truth_prec, ground_truth_weight_log, rays, beta_2, beta_3, beta_4, beta_5)
    alpha_list.append(ground_truth_alpha)
    all_depths.append(ground_truth_depth)
...
for i in loop:
    ...
    depth,_,alpha = fm_render.render(means,prec,weights_log,cameras_list[0],beta_2,beta_3,beta_4,beta_5)
...
min_depth=1.0
max_depth=10.0
ground_truth_depth = all_depths[0]
ground_truth_alpha = alpha_list[0]
