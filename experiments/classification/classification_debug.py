...
table_pose, table_dims = jax3dp3.utils.find_table_pose_and_dims(
    ...
    ransac_threshold=0.01, inlier_threshold=0.02, segmentation_threshold=0.04)
...
segmentation_img = jax3dp3.utils.segment_point_cloud_image(gt_image_above_table, threshold=0.05, min_points_in_cluster=30)
...
for (i,idx) in enumerate(object_indices):
    image = jax3dp3.render_single_object(poses[i], idx)
    weight = scorer_parallel_jit(gt_image_masked, image[None,...], r, 0.2, 1.0)[0]
    all_scores_new.append(weight)
all_scores_new = jnp.array(all_scores_new)
ps = jax3dp3.utils.normalize_log_scores(all_scores_new)
print(ps)
plt.clf()
plt.plot(ps)
