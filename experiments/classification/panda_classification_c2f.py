...
for scene_num in [1,3,4,6]:
    ...
    depth_data = data["depth"] / 1000.0
    # jax3dp3.viz.get_depth_image(depth_data, max=3000.0).save("depth.png")
    K = data["intrinsics"][0]
    ...
    depth_data = jax3dp3.utils.resize(depth_data, h, w)
    ...
    ### segment the image
    gt_image_full = t3d.depth_to_point_cloud_image(depth_data, fx,fy,cx,cy)
    ...
    def get_pose_estimation_for_segmentation(seg_id):
        ...
        gt_panels_viz = jax3dp3.c2f.multi_panel_gt_images(rgb_data, gt_image_full, gt_image_above_table, segmentation_img, h,w, max_depth, title=f"GT Images: Scene {scene_num} Frame {frame_num}", scene_id=scene_num)
        gt_panels_viz.save(f"imgs/gt_panels_scene_{scene_num}_frame_{frame_num}.png")
