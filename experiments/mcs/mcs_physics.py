...
import jax
rotation_deltas_global = jax.vmap(lambda ang: j.t3d.transform_from_axis_angle(jnp.array([1.0, 0.0, 0.0]), jnp.deg2rad(ang)))(jnp.linspace(-20.0, 20.0, 100))
...
for t in range(1,len(images)):
    ...
    for i in [*jnp.arange(NUM_OBJECTS),*jnp.arange(NUM_OBJECTS)]:
        ...
        pose_proposals_translation = pose_estimates_tiled.at[i].set(jnp.einsum("aij,ajk->aik", pose_estimates_tiled[i,...], translation_deltas_global))
