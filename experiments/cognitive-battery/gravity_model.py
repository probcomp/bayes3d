...
def gravity_model(
    video_path: str, meshes_path: str, num_objects=10, out_path: str = ""
):
    ...
    prior_parallel = jax.vmap(prior, in_axes=(0, None))

    def scorer(rendered_image, gt, r=0.1, op=0.01, ov=1):
        ...
        for _ in range(config.iterations_per_step):
            for i in range(n_objects):
                for d in config.grid_deltas:
                    ...
                    prior_score = prior_parallel(translation_proposals[:, i], jnp.array(past_poses[i]))
