...
def render_planes_multiobject_augmented(poses, shape_planes, shape_dims, h,w, fx,fy, cx,cy, offsets, scales):
    ...
    valid = jnp.all(
        jnp.abs(points_in_plane_frame[:,:,:,:,:2]) < shape_dimensions,
        axis=-1
    ) *  (points[:,:,:,:,2] > 0.0)

    idxs_pre = jnp.argmax((200000.0 - points[:,:,:,:, 2]) * valid, axis=-1)

    points2 = points[
        jnp.arange(points.shape[0])[:, None,None],
        jnp.arange(points.shape[1])[None,:, None],
        jnp.arange(points.shape[2])[None,None,:],
        idxs_pre,
        :
    ...
    valid2 = jnp.any(valid, axis=-1)
    idxs = jnp.argmax((200000.0 - points2[:,:,:, 2]) * valid2, axis=-1)


    points_final = (
        points2[
            jnp.arange(points2.shape[0])[:, None],
            jnp.arange(points2.shape[1])[None, :],
            idxs,
        ]
    )
