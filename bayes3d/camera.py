...
def camera_rays_from_intrinsics(intrinsics):
    ...
    """
    rows, cols = jnp.meshgrid(jnp.arange(intrinsics.width), jnp.arange(intrinsics.height))
    pixel_coords = jnp.stack([rows,cols],axis=-1)
    pixel_coords_dir = (pixel_coords - jnp.array([intrinsics.cx, intrinsics.cy])) / jnp.array([intrinsics.fx, intrinsics.fy])
...
def render_point_cloud_batched(point_cloud, intrinsics, NUM_PER, pixel_smudge=1):
    ...
    for i in range(num_iters):
        img = b.render_point_cloud(point_cloud[i*NUM_PER:i*NUM_PER+NUM_PER], intrinsics, pixel_smudge=pixel_smudge)
