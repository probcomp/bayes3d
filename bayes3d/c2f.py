...
def score_poses_batched(
    ...
    for batch_idx, pose_proposals_batch in enumerate(batches):
        ...
        weights = bayes3d.threedp3_likelihood_parallel_jit(
...
def score_contact_parameters(
    ...
    pose_proposals = bayes3d.scene_graph.pose_from_contact_and_face_params_parallel_jit(
        contact_param_sweep,
        face_param_sweep,
...
def get_probabilites(hypotheses):
    ...
    normalized_scores = bayes3d.utils.normalize_log_scores(scores)
...
def c2f_occlusion_viz(
    ...
    rgb_viz = bayes3d.viz.resize_image(bayes3d.viz.get_rgb_image(rgb_original, 255.0), h,w)
    all_images_overlayed = bayes3d.render_multiobject(pose_proposals, [obj_idx for _ in range(pose_proposals.shape[0])])
    enumeration_viz_all = bayes3d.viz.get_depth_image(all_images_overlayed[:,:,2], max=far)

    all_images_overlayed = bayes3d.render_multiobject(good_poses, [obj_idx for _ in range(good_poses.shape[0])])
    enumeration_viz = bayes3d.viz.get_depth_image(all_images_overlayed[:,:,2], max=far)
    overlay_viz = bayes3d.viz.overlay_image(rgb_viz, enumeration_viz)
    
    if len(ranked_high_value_seg_ids) > 0:
        image_masked = bayes3d.get_image_masked_and_complement(
            obs_point_cloud_image, segmentation_image, ranked_high_value_seg_ids[0], far
        )[0]
        best_occluder = bayes3d.viz.get_depth_image(image_masked[:,:,2],  max=far)
    else:
        best_occluder = bayes3d.viz.get_depth_image(obs_point_cloud_image[:,:,2],  max=far)

    return bayes3d.viz.multi_panel(
