...
def create_table(
    ...
    **kwargs
    ...
    surface_pose = (
        np.array([0.0, 0.0, height/2.0 - thickness/2.]),
        IDENTITY_QUAT
    )
    ...
    leg_poses = [
        (
            np.array([x, y, leg_height/2. - height/2.0]),
            IDENTITY_QUAT
        )
        for x, y in leg_xys
    ]
...
def add_mesh(
    ...
    dims, pose = bayes3d.utils.aabb(mesh.vertices)
...
def convert_to_pybullet_pose(pose):
    return np.array(pose[:3,3]), np.array(t3d.rotation_matrix_to_xyzw(pose[:3,:3]))

def convert_from_pybullet_pose(position, orientation):
    return t3d.transform_from_rot_and_pos(t3d.xyzw_to_rotation_matrix(orientation), jnp.array(position))


def set_pose_wrapped(body, pose, client=None):
    client = client or DEFAULT_CLIENT
    client.resetBasePositionAndOrientation(body, *convert_to_pybullet_pose(pose))

def get_pose_wrapped(body):
    position, orientation =  get_pose(body)
    return convert_from_pybullet_pose(position, orientation)

def get_link_pose_wrapped(body, link):
