...
class SceneGraph(
    ...
    def visualize(self, filename, node_names=None, colors=None):
        ...
        for i in range(num_nodes):
            g_out.node(
                f"{i}", node_names[i], fillcolor=matplotlib.colors.to_hex(colors[i])
            )
        ...
        for i, parent in enumerate(scene_graph.parents):
            ...
            contact_string = "contact:\n" + " ".join(
                [f"{x:.2f}" for x in scene_graph.contact_params[i]]
            )
...
def get_contact_planes(dimensions):
    return jnp.stack(
        [
            # bottom
            t3d.transform_from_pos(jnp.array([0.0, dimensions[1] / 2.0, 0.0])).dot(
                t3d.transform_from_axis_angle(jnp.array([1.0, 0.0, 0.0]), -jnp.pi / 2)
            ),
            # top
            t3d.transform_from_pos(jnp.array([0.0, -dimensions[1] / 2.0, 0.0])).dot(
                t3d.transform_from_axis_angle(jnp.array([1.0, 0.0, 0.0]), jnp.pi / 2)
            ...
            # back
            t3d.transform_from_pos(jnp.array([0.0, 0.0, dimensions[2] / 2.0])).dot(
                t3d.transform_from_axis_angle(jnp.array([1.0, 0.0, 0.0]), 0.0)
            ),
            # front
            t3d.transform_from_pos(jnp.array([0.0, 0.0, -dimensions[2] / 2.0])).dot(
                t3d.transform_from_axis_angle(jnp.array([1.0, 0.0, 0.0]), jnp.pi)
            ),
            # left
            t3d.transform_from_pos(jnp.array([-dimensions[0] / 2.0, 0.0, 0.0])).dot(
                t3d.transform_from_axis_angle(jnp.array([0.0, 1.0, 0.0]), -jnp.pi / 2)
            ),
            # right
            t3d.transform_from_pos(jnp.array([dimensions[0] / 2.0, 0.0, 0.0])).dot(
                t3d.transform_from_axis_angle(jnp.array([0.0, 1.0, 0.0]), jnp.pi / 2)
...
def iter(
    root_poses, box_dimensions, parent, child, contact_params, face_parent, face_child
):
    ...
    return root_poses[parent].dot(relative) * (parent != -1) + root_poses[child] * (
        parent == -1
    )


def poses_from_scene_graph(
    root_poses, box_dimensions, parents, contact_params, face_parent, face_child
):
    def _f(poses, _):
        new_poses = jax.vmap(iter, in_axes=(None, None, 0, 0, 0, 0, 0))(
            poses,
            box_dimensions,
            parents,
            jnp.arange(parents.shape[0]),
            contact_params,
            face_parent,
            face_child,
        )
...
def closest_approximate_contact_params(parent_contact_plane, child_contact_plane):
    ...
    pose_ = (
        t3d.inverse_pose(
            t3d.transform_from_axis_angle(jnp.array([1.0, 1.0, 0.0]), jnp.pi)
        )
        @ contact_pose
    )
    ...
    slack = (
        t3d.inverse_pose(contact_params_to_pose(inferred_contact_params)) @ contact_pose
    )
