...
class Renderer(object):
    ...
    def rasterize(self, pos_clip_ja, pos_idx, resolution):
        return _rasterize_custom_call(self, pos_clip_ja, pos_idx, resolution)
    
    def interpolate(self, pos_clip_ja, pos_idx, resolution):
        return _interpolate_custom_call(self, pos_clip_ja, pos_idx, resolution)
...
def _rasterize_custom_call(r: "Renderer", pos_clip_ja, pos_idx, resolution):
    return _build_rasterize_primitive(r).bind(pos_clip_ja, pos_idx, resolution)

@functools.lru_cache(maxsize=None)
def _build_rasterize_primitive(r: "Renderer"):
...
# =========================
@functools.partial(jax.jit, static_argnums=(0,))
def _interpolate_custom_call(r: "Renderer", pos_clip_ja, pos_idx, resolution):
    return _build_interpolate_primitive(r).bind(pos_clip_ja, pos_idx, resolution)

@functools.lru_cache(maxsize=None)
def _build_interpolate_primitive(r: "Renderer"):
    _register_custom_calls()
    # For JIT compilation we need a function to evaluate the shape and dtype of the
    # outputs of our op for some given inputs

    def _interpolate_abstract(pos_clip_ja, pos_idx, resolution):
        if (len(pos_clip_ja.shape) != 3):
            raise ValueError(f"Pass in a [num_images, num_vertices, 4] sized first input")
        num_images, num_vertices, _ = pos_clip_ja.shape
        num_triangles, _ = pos_idx.shape
    ...
    def _interpolate_lowering(ctx, pos_clip_ja, pos_idx, resolution):
        """
        Single-object (one obj represented by pos_idx) rasterization with 
        multiple poses (first dimension fo pos_clip_ja)
        dr.rasterize(glctx, pos_clip_ja, pos_idx, resolution=resolution)
        ...
        op_name = "jax_interpolate_fwd_gl"
