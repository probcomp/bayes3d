...
# ----------------------
rot_mtx_44 = torch.tensor(
    [
        [-0.9513, 0.1699, 0.2573, 0.0000],
...
if JAX_RENDERER:
    ...
    (rast_out, rast_out_db), rasterize_vjp = jax.vjp(
        jax_renderer.rasterize,
    ...
    (rast_out, rast_out_db), rasterize_vjp = jax.vjp(
        jax_renderer.rasterize, pos_clip_ja_jax, pos_idx_jax, resolution
    )
    pos_grads = rasterize_vjp((dummy_dy, dummy_ddb))[0]
    end_time = time.time()
    ...
    print(
        "JAX FWD grads (sum, min, max):",
        rast_out_db.sum().item(),
        rast_out_db.min().item(),
        rast_out_db.max().item(),
    )
    print(
        "JAX BWD (sum, min, max):",
        pos_grads.sum().item(),
    ...
    rast_out, rast_out_db = dr.rasterize(
        torch_glctx, pos_clip_ja, pos_idx, resolution=resolution
    )
    ctx = TorchCtx(
        saved_tensors=(pos_clip_ja, pos_idx, rast_out),
        saved_grad_db=True,
        saved_misc=None,
    )
    pos_grads = dr._rasterize_func.backward(ctx, dummy_dy, dummy_ddb)[
        1
    ]  # 7 outputs; all are None except pos input
    end_time = time.time()

    # print results
    print(
        "TORCH FWD (sum, min, max):",
        rast_out.sum().item(),
        rast_out.min().item(),
        rast_out.max().item(),
    )
    print(
        "TORCH FWD (sum for channels):",
        rast_out[..., 0].sum().item(),
        rast_out[..., 1].sum().item(),
        rast_out[..., 2].sum().item(),
        rast_out[..., 3].sum().item(),
    ...
    print(
        "TORCH FWD grads (sum, min, max):",
        rast_out_db.sum().item(),
        rast_out_db.min().item(),
        rast_out_db.max().item(),
    )
    print(
        "TORCH BWD (sum, min, max):",
        pos_grads.sum().item(),
        pos_grads.min().item(),
        pos_grads.max().item(),
    )
...
if JAX_RENDERER:
    ...
    # jit with dummy input
    jax_renderer.interpolate(
        jnp.zeros_like(posw_jax),
        jnp.zeros_like(rast_out),
        jnp.ones_like(pos_idx_jax),
        jnp.ones_like(rast_out_db),
        all_attributes,
    )

    start_time = time.time()
    (gb_pos, dummy), interpolate_vjp = jax.vjp(
        jax_renderer.interpolate,
        posw_jax,
        rast_out,
        pos_idx_jax,
    ...
    g_attr, g_rast, _, _, _ = interpolate_vjp((dummy_dy, dummy))
    end_time = time.time()

    # print results
    print(
        f"JAX FWD (sum, min, max): {gb_pos.sum().item(), gb_pos.min().item(), gb_pos.max().item()}"
    )
    print(
        "JAX FWD (sum for channels):",
        gb_pos[..., 0].sum().item(),
    ...
    start_time = time.time()
    gb_pos, dummy = dr.interpolate(
        posw, rast_out, pos_idx, rast_db=rast_out_db, diff_attrs="all"
    )
    ctx = TorchCtx(
        saved_tensors=(posw, rast_out, pos_idx, rast_out_db),
        saved_grad_db=None,
        saved_misc=(1, []),
    )
    grads = dr._interpolate_func_da.backward(
        ctx, dummy_dy, dummy
    )  # 6 outputs; all are None except pos input
    g_attr, g_rast = grads[0], grads[1]
    end_time = time.time()

    # print results
    print(
        f"TORCH FWD (sum, min, max): {gb_pos.sum().item(), gb_pos.min().item(), gb_pos.max().item()}"
    )
    print(
        "TORCH FWD (sum for channels):",
        gb_pos[..., 0].sum().item(),
